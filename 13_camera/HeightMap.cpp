    ...
    Entity(gameWorld)
{
    ...
    collider = std::shared_ptr<Collider>(new TerrainCollider((Entity*)this, (HeightMap*)this));
}
...
void HeightMap::OnCollision(Entity* entity)
{
    DBGASSERT(entity != nullptr);
    DBGASSERT(entity->GetCollider() != nullptr);

    if (entity->CanBeRemoved()) {
        return;
    }

    // move entity upwards if we hit the ground
    Vec3 entityPos = entity->GetPosition();
    float entityHeight = entityPos.y;
    float terrainHeight = GetHeightAt(entityPos.x, entityPos.z);
