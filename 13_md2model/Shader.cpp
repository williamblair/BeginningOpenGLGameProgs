...
void Shader::SetUniform(const std::string& name, Mat4& mat)
{
    unsigned int id = GetUniform(name);
    glUniformMatrix4fv(id, 1, false, (const GLfloat*)mat.v);
}
...
void Shader::SetUniform(const std::string& name, Vec2& vec)
{
    unsigned int id = GetUniform(name);
    glUniform2fv(id, 1, (const GLfloat*)vec.v);
}
