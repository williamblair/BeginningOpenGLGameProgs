...
GameWorld::~GameWorld()
{
    ...
    if (renderFrustum != nullptr)
    {
        delete renderFrustum;
        renderFrustum = nullptr;
    }
...
bool GameWorld::Init()
{
    ...
    if (renderFrustum == nullptr) {
        std::cout << __func__ << ": Failed to allocate render frustum" << std::endl;
        return false;
    }
    ...
    player->SetPosition(Vec3(10.0f, 0.0f, 0.0f));
    camera->AttachTo((Entity*)player);
...
void GameWorld::Render()
{
    ...
    DBG_ASSERT(GetProjectionMatrix() != nullptr);

    renderFrustum->UpdateFrustum(GetViewMatrix(),
                                 GetProjectionMatrix());
    ...
    for (Entity* entity : entities)
    {
        ...
        if (entity->GetType() == EntityType::LANDSCAPE  ||
            entity->GetCollider() == nullptr)
        {
            entity->Render();
            entity->PostRender();
        }
        else if (renderFrustum->SphereInFrustum(pos.x, pos.y, pos.z, 
